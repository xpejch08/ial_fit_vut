...
void List_Init( List *list ) {
    //initializing all pointers of list to NULL
...
void List_Dispose( List *list ) {
    if(list->firstElement != NULL){
        //variable for storing the element after our first element
        ListElementPtr tmp;

        //until our first element is'nt NULL we have to go through the list
...
void List_InsertFirst( List *list, int data ) {
    ...
    //varibale for storing our new element
    ListElementPtr tmp = malloc(sizeof (struct ListElement));
    //if malloc went wrong end function
    if(tmp == NULL){
        List_Error();
        return ;
    }
    //setting our new element
    tmp->data = data;
...
void List_DeleteFirst( List *list ) {
    ...
    //variable for storing the element after first element
    ListElementPtr tmp;
    if(list->firstElement != NULL){
        tmp = list->firstElement->nextElement;
        free(list->firstElement);
...
void List_DeleteAfter( List *list ) {
    ...
    //variable for storing the element after active element
    ListElementPtr tmp;
...
void List_InsertAfter( List *list, int data ) {
    if(list->activeElement != NULL){
        ...
        //variable for storing new element
        ListElementPtr tmp = malloc(sizeof(struct ListElement));
        //checking if malloc went succesfully, else return and end function
        if(tmp == NULL) {
            List_Error();
            return;
        }
...
void List_GetValue( List *list, int *dataPtr ) {
    //checking if list is empty, if it is, cal error function and end function
    if(list->activeElement == NULL){
        List_Error();
        return;
    }
    else{
        //sets dataPtr parameter to data of our active element
...
void List_SetValue( List *list, int data ) {
    //checking if our list is active, else ends function
    if(list->activeElement != NULL) {
...
void List_Next( List *list ) {
    if(list->activeElement != NULL) {
        //setting our active element to the one after our current active element
