...
void DLL_Init( DLList *list ) {
    //setting all element pointers to NULL to initialize an empty list
...
//tmp variable for storing pointer to the present first element, so we can free the element later
    DLLElementPtr tmp;

    while((tmp = list->firstElement)){
...
void DLL_InsertFirst( DLList *list, int data ) {
    ...
    //tmp variable for initializing new element so we can store it in our list
    ...
    //setting pointer of first element -> previous element to tmp
...
void DLL_InsertLast( DLList *list, int data ) {
    ...
    //if list is empty we set tmp as first element
    if(!list->firstElement){
        list->firstElement = tmp;

    }
        //setting pointer of lastelement->nextelement to our new element
    else{
...
void DLL_DeleteFirst( DLList *list ) {
    ...
    //if first element was active, we set the active element to NULL
    if(list->firstElement == list->activeElement){
        list->activeElement = NULL;
    }

    //variable for storing our present first element, so we can free it later
...
void DLL_DeleteLast( DLList *list ) {
    ...
    //variable for storing our present last element, so we can free it later
...
void DLL_DeleteAfter( DLList *list ) {
    //checking if list is active, or if the last element is active
    //in that case nothing happens and function returns
    if((list->activeElement == list->lastElement) || list->activeElement == NULL){
        return;
    }

    //variable to store our the element after active element so we can free it later
...
void DLL_DeleteBefore( DLList *list ) {
    //checking if list is active, or if the first element is active
    //in that case nothing happens and function returns
...
void DLL_InsertAfter( DLList *list, int data ) {
    //if our list has no active element nothing happens
    if(list->activeElement == NULL){
        return;
    }

    //variable tmp for initializing our new element
    DLLElementPtr tmp = malloc(sizeof (struct DLLElement));
    //checking if malloc went succesfully
    if(tmp == NULL){
        DLL_Error();
        return;
    }
