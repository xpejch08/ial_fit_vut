...
void Queue_Init( Queue *queue ) {
    ...
    //initializing all elements of queue to *
    for (int i = 0; i < QUEUE_SIZE; i++) {
        queue->array[i] = '*';
    }
    //setting free index and first index to 0
    queue->freeIndex = 0;
    queue->firstIndex = 0;
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    ...
    //setting dataPtr variable to the value on the first index of our queue
...
void Queue_Remove( Queue *queue ) {
    ...
    //setting first index to value of second index
...
void Queue_Enqueue( Queue *queue, char data ) {
    ...
    //inserting our data on our free index
    queue->array[queue->freeIndex] = data;
    //setting free index to the one after our present free index
